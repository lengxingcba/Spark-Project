...
rdd0 = spark.sparkContext.textFile("/test/us_data/us-counties.txt")
...
df3 = spark.sql(
    "select date,state,sum(cases) as totalCases,sum(deaths) as totalDeaths,round(sum(deaths)/sum(cases),4) as deathRate from usInfo"
    "  where date = to_date('2020-05-19','yyyy-MM-dd') group by date,state")
